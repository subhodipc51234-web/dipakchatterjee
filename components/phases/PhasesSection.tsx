// components/phases/PhasesSection.tsx
//
// Public rendering of Phases: a modular subsection of Features (not a
// first-class, independently toggleable homepage section — see
// lib/homepage-layout.ts) rendered at a fixed position right after the
// Features/Organizations/Posts run on the homepage, whenever at least
// one phase exists.
//
// Each phase gets the side-by-side editorial layout from the spec:
//   - Left column (~40%): period + title, then the summary narrative.
//   - Right column (~60%): a photo grid with lightbox expand.
//   - Mobile: stacks vertically in the same order (title/period, then
//     summary, then gallery) since the grid just collapses to one
//     column and the left column is first in document order.

import ReactMarkdown from "react-markdown";
import type { Components } from "react-markdown";
import type { Phase, PhasePhoto } from "@/types/domain";
import PhaseGallery from "./PhaseGallery";

const proseComponents: Components = {
  p: ({ children }) => <p className="leading-relaxed">{children}</p>,
};

function PhaseEntry({ phase, index }: { phase: Phase; index: number }) {
  const photos = (phase.photos as unknown as PhasePhoto[] | null) ?? [];

  return (
    <div className={`py-12 md:py-16 ${index > 0 ? "border-t border-line" : ""}`}>
      <div className="grid grid-cols-1 md:grid-cols-[0.8fr_1.2fr] gap-8 md:gap-12 items-start">
        <div>
          {phase.period && (
            <p className="text-xs font-semibold uppercase tracking-wide text-[var(--theme-primary)] mb-2">
              {phase.period}
            </p>
          )}
          <h3 className="font-display text-2xl md:text-3xl text-navy-900 leading-tight">{phase.title}</h3>

          <div className="mt-4 text-ink-600 leading-relaxed [&_p]:mb-4 last:[&_p]:mb-0">
            <ReactMarkdown components={proseComponents}>{phase.summary}</ReactMarkdown>
          </div>
        </div>

        <PhaseGallery photos={photos} />
      </div>
    </div>
  );
}

export default function PhasesSection({ phases }: { phases: Phase[] }) {
  if (phases.length === 0) return null;

  return (
    <section id="phases" className="py-16 md:py-24 bg-paper-100 border-t border-line">
      <div className="max-w-6xl mx-auto px-5 md:px-8">
        <div className="max-w-2xl mb-4">
          <p className="text-[var(--theme-primary)] font-semibold text-sm mb-3">Phases</p>
          <h2 className="font-display text-3xl md:text-4xl text-navy-900 leading-tight">
            A life in chapters &mdash; teaching, organising, and public service.
          </h2>
        </div>

        <div>
          {phases.map((phase, i) => (
            <PhaseEntry key={phase.id} phase={phase} index={i} />
          ))}
        </div>
      </div>
    </section>
  );
}
